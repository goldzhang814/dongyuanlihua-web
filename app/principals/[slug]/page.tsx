...
export default async function PrincipalDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedProducts = products.filter((product) => product.eyebrow.toLowerCase().includes(principal.shortName.toLowerCase()) || (principal.slug === "kclka" && product.id === "shoe-machinery") || (principal.slug === "xinyang" && product.id === "acetate-tow"));
  return <><SiteHeader /><main className="detail-page principal-detail"><div className="shell"><Link className="back-link" href="/principals">← All principals</Link><div className="principal-detail-head"><div className="principal-badge blue"><span>{principal.shortName}</span><small>{principal.name}</small><b>{principal.year}</b></div><div><span className="eyebrow">{principal.eyebrow}</span><h1>{principal.title}</h1><p className="detail-lead">{principal.description}</p></div></div><section className="principal-detail-body"><h2>Products and market support</h2><div className="catalog-grid">{relatedProducts.map((product) => <Link className="catalog-card" href={`/products/${product.id}`} key={product.id}><div className="catalog-art">{product.image && <Image unoptimized src={product.image} alt={product.title} fill sizes="(max-width: 800px) 100vw, 33vw" />}</div><span className="eyebrow">{product.category}</span><h2>{product.title}</h2><p>{product.summary}</p></Link>)}</div></section></div></main><SiteFooter /></>;
}
